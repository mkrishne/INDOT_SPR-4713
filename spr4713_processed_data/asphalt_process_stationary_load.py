...
asphalt_static_load_folders = ["17-22 C","23-28 C"]

file_info_column_names = ["Filename", "Load Folder", "Scan Point", "Starting Timestamp"]
...
for load_folder in asphalt_static_load_folders:
    folder_path = "spr4713_stationary_process\\Asphalt Static Load\\" + load_folder
    os.chdir(folder_path)
    static_load_fldr_txt_files =  glob.glob('*.txt')
    os.chdir(MAIN_DIR)
    for filename in static_load_fldr_txt_files:
        if(filename == "Stationary Load Pt 5 ET.txt"): #data is not properly captured in this file. Hence skipped
            continue
        file_info_data_per_day = {}
        file_info_data_per_day['Filename'] = filename
        file_info_data_per_day["Load Folder"] = load_folder
        file_info_data_per_day["Scan Point"] = filename.replace("Stationary Load ","").replace(".txt","")
        ...
        for column_num in range(0,len(data_columns)):
            ...
            if(sensor_type == "asphalt"):  
               ...
               if((load_folder == "17-22 C") and (asg_entry_num == 1)):
                    asg_processed_csv_data.to_csv('asph_static_asg_processed_csv_data.csv', mode='a', index=False, header=True)
               else:
                    asg_processed_csv_data.to_csv('asph_static_asg_processed_csv_data.csv', mode='a', index=False, header=False)
                ...
                if((load_folder == "17-22 C") and (csg_entry_num == 1)):
                    csg_processed_csv_data.to_csv('asph_static_csg_processed_csv_data.csv', mode='a', index=False, header=True)
                else:
                    csg_processed_csv_data.to_csv('asph_static_csg_processed_csv_data.csv', mode='a', index=False, header=False)
                ...
                if((load_folder == "17-22 C") and (pc_entry_num == 1)):
                    pc_processed_csv_data.to_csv('asph_static_pc_processed_csv_data.csv', mode='a', index=False, header=True)
                else:
                    pc_processed_csv_data.to_csv('asph_static_pc_processed_csv_data.csv', mode='a', index=False, header=False)
                ...
                if((load_folder == "17-22 C") and (thermo_entry_num == 1)):
                    thermo_processed_csv_data.to_csv('asph_static_thermo_processed_csv_data.csv', mode='a', index=False, header=True)
                else:
                    thermo_processed_csv_data.to_csv('asph_static_thermo_processed_csv_data.csv', mode='a', index=False, header=False)
        
file_info_csv_data.to_csv('asph_static_sensor_file_info.csv', mode='a', index=False, header=True)    
