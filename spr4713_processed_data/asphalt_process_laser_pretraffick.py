...
#pre traffick
pre_traffick_main_folder = "Asphalt Traffic Testing\Laser Profile\Pre Traffick 07-01-22\\0 Passes"
os.chdir(pre_traffick_main_folder)
laser_pre_traffick_pts = glob.glob('*')
print(laser_pre_traffick_pts)
os.chdir(MAIN_DIR)

file_column_names = ["Pre Traffick Folder","Filename","Order of Scan", "Starting Timestamp","Ending Timestamp","Laser beam offset(mm)", "Laser beam offset locn", \
...
for pre_traffick_pt in laser_pre_traffick_pts: #laser_pre_traffick_pts = ["Pt 0-ET", "Pt 1-ET", "Pt 4-ET"]
    ...
    for file_substring in pre_traffick_files_string_to_grep:
        ...
        dict_file_info_per_row["Filename"] = filename
        dict_pre_traffick_per_row["Filename"] = filename
        ...
        if(pre_traffick_reading_count == 1):
            pre_traffick_csv_data.to_csv('processed_asphalt_pre_traffick_data.csv', mode='a', index=False, header=True)
        else:
            pre_traffick_csv_data.to_csv('processed_asphalt_pre_traffick_data.csv', mode='a', index=False, header=False)
    
        if(filename_count == 1):
            file_info_csv_data.to_csv('asphalt_pre_traffick_file_info.csv', mode='a', index=False, header=True)
        else:
            file_info_csv_data.to_csv('asphalt_pre_traffick_file_info.csv', mode='a', index=False, header=False)
    print("")
